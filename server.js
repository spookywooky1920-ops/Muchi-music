...
// Optional: point the web app at a remote API (e.g. the deployed Render service).
// When set, server.js injects window.MUCHI_API_BASE into served HTML so the
// frontend fetches /api/* from that origin instead of same-origin.
const MUCHI_API_BASE = String(process.env.MUCHI_API_BASE || "").trim().replace(/\/+$/, "");
const MUCHI_API_BASE_OK = !MUCHI_API_BASE || /^https?:\/\//i.test(MUCHI_API_BASE);

// ── Google Sign-In + YouTube (server-side OAuth) ──────────────────────
// Env contract (Render):
//   GOOGLE_CLIENT_ID      web OAuth client id (public)
//   GOOGLE_CLIENT_SECRET  web OAuth client secret (NEVER in repo/client)
//   GOOGLE_REDIRECT_URI   https://muchi-music.onrender.com/api/auth/google/callback
//   MUCHI_SESSION_SECRET  random string used to sign session tokens
// All user tokens live here in memory — never sent to the client.
const GOOGLE_CLIENT_ID = String(process.env.GOOGLE_CLIENT_ID || "").trim();
const GOOGLE_CLIENT_SECRET = String(process.env.GOOGLE_CLIENT_SECRET || "").trim();
const GOOGLE_REDIRECT_URI = String(process.env.GOOGLE_REDIRECT_URI || "").trim();
const MUCHI_SESSION_SECRET = String(process.env.MUCHI_SESSION_SECRET || "").trim();
const GOOGLE_AUTH_ON = !!(GOOGLE_CLIENT_ID && GOOGLE_CLIENT_SECRET && GOOGLE_REDIRECT_URI && MUCHI_SESSION_SECRET);

const SESSION_TTL_MS = 30 * 24 * 60 * 60 * 1000; // 30 days
const sessions = new Map();       // sid -> session object
const oauthStates = new Map();    // state -> { step, sid, exp }
const ytCache = new Map();        // sid -> { liked, playlists, detail: Map(plId), at }

// ── Session persistence ──────────────────────────────────────────────
// Render's free tier sleeps/starts instances, which would wipe in-memory
// sessions and log everyone out. Sessions + in-flight OAuth states are
// mirrored to a small JSON file (0600, outside public/, never committed)
// and reloaded at boot, so sign-in survives sleep/wake and redeploys.
// The file lives only on the server; tokens are never sent to clients.
const SESSION_FILE = path.join(__dirname, ".muchi-sessions.json");
let persistTimer = null;
function persistSessions() {
  persistTimer = null;
  try {
    const o = { sessions: {}, states: {} };
    for (const [sid, s] of sessions) o.sessions[sid] = s;
    for (const [st, v] of oauthStates) o.states[st] = v;
...
function schedulePersist() {
  ...
  persistTimer = setTimeout(persistSessions, 400);
}
function loadSessions() {
  try {
    const o = JSON.parse(fs.readFileSync(SESSION_FILE, "utf8"));
    const now = Date.now();
    if (o && o.sessions) {
      for (const [sid, s] of Object.entries(o.sessions)) {
        if (s && s.sid === sid && (now - s.at) < SESSION_TTL_MS) sessions.set(sid, s);
      }
    }
    if (o && o.states) {
      for (const [st, v] of Object.entries(o.states)) {
        if (v && v.exp > now) oauthStates.set(st, v);
      }
    }
  } catch {}
}
...
function hmac(data) {
  return crypto.createHmac("sha256", MUCHI_SESSION_SECRET).update(data).digest("base64url");
}
function sessionToken(sid) {
  return sid + "." + hmac(sid);
}
function sessionFromToken(token) {
  if (!token || !MUCHI_SESSION_SECRET) return null;
  ...
  if (!s) return null;
  if (Date.now() - s.at > SESSION_TTL_MS) { sessions.delete(sid); schedulePersist(); return null; }
  return s;
}
function readSession(req) {
  const h = req.headers || {};
  const auth = h.authorization || "";
  if (auth.startsWith("Bearer ")) return sessionFromToken(auth.slice(7).trim());
  const cookies = String(h.cookie || "");
  const m = cookies.match(/(?:^|;\s*)muchi_sid=([^;]+)/);
  if (m) return sessionFromToken(decodeURIComponent(m[1]));
  return null;
}
...
function ytTrack(item, prefix) {
  ...
  // For playlistItems, item.id is the PLAYLIST-ITEM id, not the video id —
  // the real video id lives in contentDetails.videoId (or snippet.resourceId).
  // For videos.list, contentDetails.videoId is absent, so we fall back to item.id.
...
async function googleExchange(code) {
  try {
    ...
    return await r.json();
  } catch (e) { console.error("google token exchange error", e); return null; }
}
async function googleRefresh(session) {
  ...
  try {
    ...
    if (!r.ok) { session.yt = null; schedulePersist(); return false; }
    const j = await r.json();
    session.yt.access = j.access_token;
    session.yt.expiresAt = Date.now() + (Number(j.expires_in || 3600) * 1000);
    schedulePersist();
    return true;
  } catch (e) { console.error("google refresh error", e); return false; }
}
async function ytApi(session, pathAndQuery) {
  const base = "https://www.googleapis.com/youtube/v3/";
  const doFetch = async () => {
    return fetch(base + pathAndQuery, { headers: { Authorization: "Bearer " + session.yt.access } });
  };
  let r = await doFetch();
  if (r.status === 401 && (await googleRefresh(session))) r = await doFetch();
  if (!r.ok) {
    // Surface the REAL cause in server logs (never log tokens).
    try {
...
function setSessionCookie(res, sid) {
  res.setHeader("Set-Cookie", `muchi_sid=${encodeURIComponent(sid)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${Math.floor(SESSION_TTL_MS / 1000)}`);
}
function makeOAuthUrl(scope, state, extra) {
...
// --- SSRF guard for the URL-proxy endpoints (/api/img, /api/stream, /api/audius/file) ---
// Blocks private / loopback / link-local targets so the public server can't be used
// to reach internal networks or cloud metadata services. Public hosts only.
...
async function assertPublicUrl(src) {
  ...
  if (!addrs || !addrs.length) throw new Error("dns failed");
...
// Finds the "load more" token inside a playlist browse response. Prefers the
// token inside the actual track list (playlistVideoListRenderer) so we keep
// paging the playlist itself, not some side section.
...
async function youtubePlaylistTracks(playlistId) {
  ...
  async function browse(clientName, clientVersion, origin) {
    ...
    // Follow "load more" continuations so the whole playlist comes back
    // (deduped), not just the first ~100 songs.
...
async function pipeUrl(req, res, src, accept) {
  try {
    ...
    return sendJSON(res, 400, { error: e.message || "bad url" });
  }
  const ctrl = new AbortController();
...
async function radioSearch(query, limit = 24, quality, codec) {
  ...
  // Quality tiers (mirrors the app's Stream quality setting):
  // low -> <=96 kbps, standard -> >=128, high -> >=192, highest -> >=320
  let floor = 0;
  let ceil = 0;
  ...
  // Always prefer higher-bitrate stations within the chosen tier.
...
// Picks the playlist hit whose title best matches the search query, so a shelf
// opens a real playlist instead of ad-hoc search results.
function pickPlaylistHit(pls, query) {
...
// Resolves a real YouTube Music playlist for a query (e.g. "trending music
// hits"), so playlists open with their full track list.
async function resolveShelfPlaylist(query, gl) {
...
// "Made for you": curated real YouTube playlists + taste mixes.
const FY_QUERIES = [
...
function buildForYouPlaylists(fyRes) {
  return [
    ...FY_QUERIES.map((f, i) => {
...
async function handleApi(req, res, url) {
  ...
  // ── Google Sign-In + YouTube (additive; no-op when not configured) ──
  if (p === "/api/auth/status") {
    const s = readSession(req);
    const ytOn = !!(s && s.yt && s.yt.access);
    return sendJSON(res, 200, {
      configured: GOOGLE_AUTH_ON,
      signedIn: !!s,
  ...
  if (p === "/api/auth/google/url" || p === "/api/auth/youtube/url") {
    if (!GOOGLE_AUTH_ON) return sendJSON(res, 503, { error: "Google auth not configured on the server" });
    const platform = url.searchParams.get("platform") === "native" ? "native" : "web";
    const s = readSession(req);
    const step = p.indexOf("youtube") >= 0 ? "youtube" : "signin";
    const state = crypto.randomBytes(18).toString("hex");
    oauthStates.set(state, { step, sid: s ? s.sid : null, platform, exp: Date.now() + 10 * 60 * 1000 });
    schedulePersist();
    ...
    return sendJSON(res, 200, { url: makeOAuthUrl(scope, state, extra) });
  }
  ...
  if (p === "/api/auth/google/callback") {
    ...
    if (oauthErr || !st || st.exp < Date.now() || !code) return authRedirect(res, errHome);
    const tok = await googleExchange(code);
    if (!tok) return authRedirect(res, errHome);
    const id = decodeIdToken(tok.id_token);
    if (!id || id.aud !== GOOGLE_CLIENT_ID) return authRedirect(res, errHome);
    ...
    schedulePersist();
    if (platform === "native") return authRedirect(res, home + "?token=" + encodeURIComponent(sessionToken(sid)));
    setSessionCookie(res, sid);
    return authRedirect(res, home);
  }

  if (p === "/api/auth/youtube/callback") {
    const code = url.searchParams.get("code") || "";
    const state = url.searchParams.get("state") || "";
    const oauthErr = url.searchParams.get("error") || "";
    const st = oauthStates.get(state);
    ...
    if (oauthErr || !st || st.exp < Date.now() || !code) return authRedirect(res, errHome);
    const s = readSession(req) || (st.sid ? sessions.get(st.sid) : null);
    if (!s) return authRedirect(res, errHome);
    const tok = await googleExchange(code);
    if (!tok) return authRedirect(res, errHome);
    s.yt = {
    ...
    schedulePersist();
    if (platform === "native") return authRedirect(res, home);
    return authRedirect(res, home);
  }
  ...
  if (p === "/api/auth/signout") {
    ...
    const m = cookies.match(/(?:^|;\s*)muchi_sid=([^;]+)/);
    if (m) {
      res.setHeader("Set-Cookie", "muchi_sid=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0");
    }
    return sendJSON(res, 200, { ok: true });
  }

  if (p === "/api/auth/youtube/disconnect") {
    const s = readSession(req);
    if (!s) return sendJSON(res, 401, { error: "auth" });
    if (s.yt && s.yt.access) revokeGoogle(s.yt.access);
    s.yt = null;
    ytCache.delete(s.sid);
    schedulePersist();
  ...
  if (p === "/api/youtube/liked" || p === "/api/youtube/playlists" || p === "/api/youtube/playlist") {
    const s = readSession(req);
    if (!s) return sendJSON(res, 401, { error: "auth" });
    if (!s.yt || !s.yt.access) return sendJSON(res, 401, { error: "youtube" });
    const sid = s.sid;
    const cache = ytCache.get(sid) || { liked: null, playlists: null, detail: new Map(), at: 0 };
    if (!ytCache.has(sid)) ytCache.set(sid, cache);
    const fresh = (obj) => obj && (Date.now() - obj.at) < 60 * 1000;
    try {
      if (p === "/api/youtube/liked") {
        if (!fresh(cache.liked)) {
          const pages = [];
          let pageToken = "";
          for (let i = 0; i < 4; i++) {
            const q = new URLSearchParams({ part: "snippet,contentDetails", myRating: "like", maxResults: "50" });
            if (pageToken) q.set("pageToken", pageToken);
            const j = await ytApi(s, "videos?" + q.toString());
        ...
        return sendJSON(res, 200, { tracks: cache.liked.tracks, truncated: cache.liked.truncated });
      }
      if (p === "/api/youtube/playlists") {
        if (!fresh(cache.playlists)) {
          const out = [];
          let pageToken = "";
          for (let i = 0; i < 4; i++) {
            const q = new URLSearchParams({ part: "snippet,contentDetails", mine: "true", maxResults: "50" });
            if (pageToken) q.set("pageToken", pageToken);
            const j = await ytApi(s, "playlists?" + q.toString());
            if (!j) throw new Error("yt");
        ...
        return sendJSON(res, 200, { playlists: cache.playlists.items });
      }
      const plId = url.searchParams.get("id") || "";
      if (!plId) return sendJSON(res, 400, { error: "Missing playlist id" });
      ...
      if (!fresh(det)) {
        ...
        for (let i = 0; i < 4; i++) {
          ...
          if (pageToken) q.set("pageToken", pageToken);
          const j = await ytApi(s, "playlistItems?" + q.toString());
          if (!j) throw new Error("yt");
          pages.push(...(j.items || []));
          pageToken = j.nextPageToken || "";
          if (!pageToken) break;
      ...
      return sendJSON(res, 200, { tracks: det.tracks, truncated: det.truncated });
    } catch (e) {
      if (e && e.message === "yt") {
        // Refresh failed or API denied → treat as disconnected so the app
        // can prompt the user to reconnect (graceful, never a crash).
        if (!s.yt) ytCache.delete(sid);
        return sendJSON(res, 401, { error: "youtube" });
      }
  ...
  if (p === "/api/home") {
    ...
    try {
      globalPart = await cached(`home:english:v5:${utcDay()}`, 86400000, async () => {
        ...
        // "Made for you" playlists: real YouTube playlists (Trending, Hit
        // Releases, Top 50, Dance, Chill, Workout, Indie, Throwback) +
        // taste-driven mixes. Real ones resolve once per day; each entry
        // stays visible even if its playlist resolution fails.
        ...
        // Never cache a fully-empty home — one bad provider day would leave
        // every shelf empty for 24 hours. Throw so the catch below returns a
        // fresh (uncached) empty shell and the next request retries.
        const total =
      ...
      // Providers are down (or still warming up): never leave "Made for
      // you" empty — ship the curated set without resolved playlist ids;
      // the client opens these by query instead.
  ...
  if (p === "/api/shelf") {
    ...
    const cap = full ? 100 : 18; // full catalog honors "top 100" shelves
    try {
      const tracks = await cached(`shelf:${full ? "full" : "row"}:${id}:${q}:${gl}:${utcDay()}`, 86400000, async () => {
        // Heavy multi-source search first; if the providers refuse, fall back to
        // the fast single-request search so the catalog still fills.
        let rows = [];
        try {
        ...
        // Never cache an empty result — otherwise one bad provider day would
        // poison the shelf for 24 hours.
      ...
      // Providers unreachable (or rate-limited) — degrade to an empty catalog
      // instead of a raw 500, matching /api/discover & friends.
      return sendJSON(res, 200, {
        id: id || (shelf && shelf.id) || "",
        title: (shelf && shelf.title) || q,
        tracks: [],
...
function serveStatic(req, res, urlPath) {
  ...
  fs.stat(file, (err, st) => {
    ...
    if (ext === ".html" && MUCHI_API_BASE && MUCHI_API_BASE_OK) {
      // Inject the remote API base into the shell (used by the live preview /
      // cross-origin deployments). Default (no env) keeps same-origin behavior.
      fs.readFile(file, (e, buf) => {
        if (e) return send(res, 500, "read error");
        const tag = `<script>window.MUCHI_API_BASE=${JSON.stringify(MUCHI_API_BASE)};</script>`;
        res.end(buf.toString("utf8").replace("<head>", "<head>" + tag));
      });
      return;
    }
