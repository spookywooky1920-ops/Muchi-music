(() => {
  ...
  function toast(msg, show) {
    ...
    toast._t = setTimeout(() => showEl(el, false), 2800);
  }
  ...
  async function api(path, timeoutMs = 18000) {
  ...
  function toggleFollow(track) {
    ...
    if (isFollowing(track)) {
      ...
      saveFollowing();
      toast(`Unfollowed ${artistName(track)}`);
    } else {
      ...
      saveFollowing();
      toast(`Following ${artistName(track)}`);
  ...
  async function downloadTrack(t) {
    ...
    toast("Saving track…");
    try {
      const res = await fetch(`/api/audius/file/${encodeURIComponent(t.trackId)}`);
  ...
  function applyYtQuality() {
    ...
    const level = q === "low" ? "medium" : "hd1080";
    try {
  ...
  function qualityLabel() {
    ...
    return auto ? `Auto · ${r}` : r === "low" ? "Low" : "High";
  }
  ...
  function looksLikeSong(t) {
    ...
    if (/\b(episode|podcast|trailer|full movie|gameplay|nato|imran khan)\b/i.test(`${title} ${artist}`)) return false;
    return true;
  }
  ...
  function skipFailed(msg) {
    ...
    failSkipAt = now;
    failSkip += 1;
    if (failSkip === 1) toast(msg || "Could not play this track");
    if (failSkip >= 3) {
  ...
  function createYT(initialId) {
    ...
    const opts = {
      ...
      playerVars: {
        ...
        fs: 1,
        vq: resolvedQuality() === "low" ? "medium" : "hd1080",
      },
      events: ytEvents(),
    };
  ...
  function updateProgress() {
    ...
    if (!$("seek").matches(":active")) $("seek").value = d ? Math.round((p / d) * 1000) : 0;
  ...
  function themeFromTrack(t) {
    ...
    const raw = artUrl(t);
    const src = raw.startsWith("http") ? `/api/img?url=${encodeURIComponent(raw)}` : raw;
  ...
  function renderChrome() {
    const t = current();
    $("coverArt").src = t ? artUrl(t) : "/cover-default.png";
    themeFromTrack(t);
  ...
  function skeleton() {
    return `<div class="sk"></div><div class="section"><div class="row">${"<div class='sk' style='height:210px'></div>".repeat(6)}</div></div>`;
  }
  ...
  function commitCrop() {
    ...
    if (crop.kind === "plCover" || crop.kind === "plBanner") {
      ...
      save("aura.playlists", state.playlists);
      closeCrop();
      toast(cover ? "Cover saved" : "Banner saved");
      if (state.view === "library") render();
      return;
    }
  ...
  function renderHome() {
    ...
    const ytOk = shelves.some((s) => s.tracks && s.tracks.length) || (local && local.length);
    const auOk = !!(h.audius && h.audius.length);
    const rdOk = !!(h.radio && h.radio.length);
    return `
      ${homeBarHTML()}
      ...
      <div class="status-row">
        <span class="status-pill ${ytOk ? "" : "warn"}"><i></i>YouTube ${ytOk ? "ready" : "slow"}</span>
        <span class="status-pill ${auOk ? "" : "bad"}"><i></i>Audius ${auOk ? "ready" : "offline"}</span>
      ...
      <div class="section">
        ...
        ` : `
        <div class="moods">
          ${personalizeMoods(h.moods || []).map((m) => `<button class="mood" data-mood="${escapeAttr(m.query)}" style="--mood:${m.color};background:${m.color}">${escapeHTML(m.title)}</button>`).join("")}
        </div>`}
      </div>
  ...
  function renderLibrary() {
    ...
    let body = "";
    if (f === "playlists") {
      body = likedRow + (playlistRows || `<p class="empty">Create a playlist with the + button.</p>`);
    } else if (f === "artists") {
      ...
      body = likedRow + playlistRows + artistRows;
  ...
  function installApkUpdate() {
    ...
    if (isMuchiApp() && window.MuchiAndroid && MuchiAndroid.installUpdate && url) {
      ...
      try { MuchiAndroid.installUpdate(url); } catch { toast("Could not start download"); }
      return;
    }
    if (url) {
    ...
    return `
      ...
      <div class="settings">
        <div class="set-card">
          ...
          <label class="set-row">
            <div><strong>Stream quality</strong><p>YouTube and radio. Audius is always 320 kbps.</p></div>
            <select id="setQuality">
              <option value="auto" ${(p.quality || "auto") === "auto" ? "selected" : ""}>Auto · network</option>
              <option value="low" ${p.quality === "low" ? "selected" : ""}>Low · save data</option>
              <option value="high" ${p.quality === "high" ? "selected" : ""}>High · best</option>
            </select>
          </label>
          <label class="set-row">
      ...
      <div class="detail-page">
        ...
        <div class="detail-hero">
          ...
          <div class="detail-copy">
            ...
            <div class="lib-hero-actions">
              <button class="filled-btn" id="detailPlay" type="button"><span class="material-symbols-outlined filled">${playingThis && state.playing ? "pause" : "play_arrow"}</span> ${playingThis && state.playing ? "Playing" : playingThis ? "Resume" : "Play"}</button>
  ...
  function bindView() {
    viewEl.querySelectorAll("[data-more]").forEach((el) => {
      el.addEventListener("click", (ev) => {
        ...
        if (!track) { toast("Couldn't open options"); return; }
    ...
    const setQuality = viewEl.querySelector("#setQuality");
    if (setQuality) {
      setQuality.addEventListener("change", () => {
        state.prefs.quality = setQuality.value;
        savePrefs();
        applyYtQuality();
        toast(state.prefs.quality === "auto" ? `Quality follows the network · now ${resolvedQuality()}` : state.prefs.quality === "low" ? "Low quality — YouTube + radio" : "High quality — YouTube + radio");
      });
    }
  ...
  function paintNav(fromBack) {
    ...
    if (document.startViewTransition) {
      try { document.startViewTransition(paint); return; } catch {}
    }
    paint();
  }
  function applyNav(s) {
  ...
  function openArtistFromTrack(t) {
    ...
    if (!name || name === "YouTube" || name === "Live radio") {
      toast("No artist name on this song");
      return;
    }
  ...
  async function openArtistProfile(artist) {
    ...
    try {
      ...
      toast("Couldn't load the full catalogue");
    }
    render();
  }
  ...
  async function openCatalogPlaylist(meta) {
    if (!meta) return;
    rememberScroll();
    const preview = Array.isArray(meta.tracks) ? meta.tracks.slice() : [];
    const playlistId = meta.playlistId || "";
    const fallbackQ = meta.query || meta.title || "";
    const shelfId = meta.shelfId || "";
    const needFill = !!(shelfId || playlistId || fallbackQ);
    ...
    if (got.length) {
      state.catalogPlaylist.tracks = got;
      if (!state.catalogPlaylist.artwork && got[0]) state.catalogPlaylist.artwork = got[0].artwork;
    }
    state.catalogPlaylist.loading = false;
    render();
    if (!state.catalogPlaylist.tracks.length) toast("Couldn't open that playlist");
  }
  ...
  async function loadHome(force) {
    ...
    render();
    try {
      state.home = await api(`/api/home?${glq()}`, 22000);
    } catch (e) {
      state.home = {
        moods: [],
        day: utcDayClient(),
  ...
  async function loadForYou() {
    ...
    try {
      ...
      const data = await api(`/api/for-you?${qs}&${glq()}`);
  ...
  (function startSparks() {
    ...
    function spawnAmbient() {
      ...
      if (state.view !== "home" || sparkBits.length > 12) return;
      const p = dpr();
      sparkBits.push({
        x: Math.random() * innerWidth * p,
        y: (innerHeight + 12) * p,
        vx: (Math.random() - 0.5) * 0.5 * p,
        vy: -(0.25 + Math.random() * 0.7) * p,
        life: 1,
        decay: 0.0012 + Math.random() * 0.001,
        g: ambient[Math.floor(Math.random() * ambient.length)],
        s: 12 + Math.random() * 14,
        hue: Math.random() > 0.45 ? 155 + Math.random() * 40 : 330 + Math.random() * 25,
      });
    }
    let sparkOn = true;
    function tick() {
      ...
      if (!mobile && state.view === "home" && sparkBits.length < 10 && Math.random() < 0.01) spawnAmbient();
      ...
      for (let i = sparkBits.length - 1; i >= 0; i--) {
        ...
        ctx.globalAlpha = Math.max(0, b.life) * 0.72;
